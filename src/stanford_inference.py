...
def main():
    ...
    model = torch.load('final_deep_regressor.pkl')
    model = model.eval().cuda()

    for (image, basename) in tqdm(data_loader):
        image = image[0]
        N = image.shape[0]
        print(image.shape)

        # just run on every slice
        # preds = [[]] * N

        # get predictions
        for i in  tqdm(range(N)):
            outputs = model([image[i, ...].unsqueeze(0).cuda()])
            output = outputs[0]

            # NMS and filtering by high confidence – testing visualizing more boxes of lower confidence
            inds = torch.Tensor([True] * len(output['scores'])) > 0.4
            nms_inds = nms(output['boxes'][inds], output['scores'][inds], 0.5)

            output = {
                'boxes' : output['boxes'][inds][nms_inds].cpu(),
                'scores' : output['scores'][inds][nms_inds].cpu(),
            }

            # preds[i].append(output)
            with torch.no_grad():
                fig, ax = plt.subplots()
                ax.imshow(image[i, ...].numpy())
                for box in output['boxes'].numpy():
                    rect = patches.Rectangle((box[0], box[1]), box[2] - box[0], box[3] - box[1],  linewidth=1, edgecolor='r', facecolor='none')
                    ax.add_patch(rect)
                ax.set_title(f'model number 1, image {basename} slice {i}')

                dp = f'./predictions/first-inference-run/{basename}/'
                if not os.path.exists(dp):
                    os.mkdir(dp)
                fig.savefig(f'{dp}/{i}.png', dpi=400)
                plt.close('all')
