...
class FracNetTrainDataset(Dataset):
    def __init__(
        ...
        self.public_id_list = sorted([x.split("-")[0] for x in os.listdir(image_dir)])
    ...
    def _get_pos_centroids(label_arr):
        centroids = [
            tuple([round(x) for x in prop.centroid]) for prop in regionprops(label_arr)
        ]
    ...
    def _get_neg_centroids(self, pos_centroids, image_shape):
        ...
        if num_pos < self.num_samples // 2:
            spine_neg_centroids = self._get_spine_neg_centroids(
                image_shape, self.crop_size, self.num_samples - 2 * num_pos
            )
        else:
            spine_neg_centroids = self._get_spine_neg_centroids(
                image_shape, self.crop_size, num_pos
            )
    ...
    def _get_roi_centroids(self, label_arr):
        if self.train:
            ...
            roi_centroids = [
                list(range(0, x, y // 2))[1:-1] + [x - y // 2]
                for x, y in zip(label_arr.shape, self.crop_size)
            ]
            roi_centroids = list(product(*roi_centroids))

        roi_centroids = [
            tuple([int(x) for x in centroid]) for centroid in roi_centroids
        ]
    ...
    def _crop_roi(self, arr, centroid):
        ...
        src_beg = [
            max(0, centroid[i] - self.crop_size // 2) for i in range(len(centroid))
        ]
        src_end = [
            min(arr.shape[i], centroid[i] + self.crop_size // 2)
            for i in range(len(centroid))
        ]
        dst_beg = [
            max(0, self.crop_size // 2 - centroid[i]) for i in range(len(centroid))
        ]
        dst_end = [
            min(arr.shape[i] - (centroid[i] - self.crop_size // 2), self.crop_size)
            for i in range(len(centroid))
        ]
        roi[
            dst_beg[0] : dst_end[0],
    ...
    def __getitem__(self, idx):
        ...
        image_rois = torch.tensor(
            np.stack(image_rois)[:, np.newaxis], dtype=torch.float
        )
...
class FracNetInferenceDataset(Dataset):
    ...
    def _get_centers(self):
        dim_coords = [
            list(range(0, dim, self.crop_size // 2))[1:-1] + [dim - self.crop_size // 2]
